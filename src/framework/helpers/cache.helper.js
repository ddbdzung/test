...
import { CACHE_PREFIX } from '@/framework/constants/common.constant'
...
export const setCache = async options => {
  ...
  try {
    ...
    const key = `${CACHE_PREFIX}_${config.service}:${keyParts.join(':')}`

    // ✅ Serialize data với error handling
    let serializedData
    try {
